...
const COLORS = ['#f9ab00', '#ea4335', '#ec4899', '#8b5cf6', '#3b82f6', '#34a853', '#78716c'];
...
const Tip = ({ active, payload }) => {
  ...
  return (
    <div style={{ background: '#1a1a24', border: '1px solid rgba(255,255,255,.15)', borderRadius: 8,
      ...
      <strong>{d.name}</strong>: {mins}m {secs}s away
    </div>
  );
};

export default function AwayTimeChart({ data }) {
  if (!data || Object.keys(data).length === 0) {
    return (
      <div style={{ height: 210, display: 'flex', alignItems: 'center', justifyContent: 'center',
                    color: '#9aa0a6', fontSize: 14 }}>
        Waiting for away time data...
      </div>
    );
  }
